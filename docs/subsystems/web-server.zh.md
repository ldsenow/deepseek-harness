...
`host` 只接受 `127.0.0.1`（默认姿态）和 `0.0.0.0`（刻意的网络暴露）。两个 TLS 路径都配置时，`node:http` 换成 `node:https`，路由、upgrade 与拆卸语义完全一致——只配置一半或材料不可读会拒绝激活——材料来源属于组合应用（[dsh-web-app](../../packages/bundle/web-app/README.md) 为 LAN 服务生成持久的自签名证书对）。本包内没有认证或 origin 策略：`/api` 一族的放行（浏览器信任栅栏与配对 token）由 connection 插件持有，因此非回环绑定会把所有没有这类所有者的路由暴露给该网络。dist 位置是认领席位的前端插件的组装事实。

## 服务

`WebServer`（`ctx.webServer`）在激活时立即监听；监听失败（EADDRINUSE 等）会使初始化被拒绝，启动进程会报告失败的 fiber。`register(route)` 添加一条具名路由并返回其 disposer；重复的 `(kind, path)` 抛出异常，因为路由模式是组合层约定，冲突即配置错误。`tapIndex(transform)` 添加一个纯 HTML 到 HTML 转换函数，按注册顺序应用于每个 index 响应（`/` 和每次 SPA 回退），并接收该次响应的脚本 nonce；[dsh-client-modules](../../packages/client/modules) 用它注入启动 manifest（元数据清单），并带上 nonce，使服务方的 Content-Security-Policy 放行它。`port` 读取监听端口，包括 `config.port` 为 0 时操作系统分配的端口；`scheme` 在配置了 TLS 材料时读取 `'https'`，否则读取 `'http'`。
