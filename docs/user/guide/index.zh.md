...
已配对的远程设备可以创建会话并运行 agent，但配置面（设置、凭据、原生对话框）仍只在宿主机本机可用。流量经过认证与加密，但证书是自签名的——请在你信任的网络上使用，或在外出访问时用 Tailscale 等 VPN 打底。经该 VPN 访问时仍按上文绑定网络；不要把服务器放在反向代理之后，代理从 `127.0.0.1` 发起连接，会让每个转发请求看起来都来自本机而跳过 token。本地连接被豁免的理由，正是它们被信任的理由——本机上的进程本来就能以你的身份行事——这也意味着：在与他人共用的机器上，任何能访问回环端口的人无需 token 即拥有该访问权。
