...
- **特权端点集与 `loopback` 权威通道** 即使对已认证调用方也仍钉在回环 peer：配对认证的是设备，配置面还额外要求人在机器旁。钉定运行在每个 `/api` 端点都会经过的那一个点上，位于网关拦截器与 API Proxy 回退之间的抉择之前。只在回退里施加它会让钉定取决于路由顺序，因为网关先认领自己的端点，而被认领的端点根本不会到达回退。
- **`/api` 承载的两种端点形式分类方式不同，因为只有其中一种是封闭集合。** API Proxy 的点号形式方法逐个列名。Typert 网关则认领任何活跃 `TypertRemoteService` 暴露的每一个 `namespace/method`，因此该空间会随每一个 `./typert` 导出而增长，逐方法的列表会让每个新端点默认可从 LAN 到达——`dynamicCordisRunner` 的十二个方法正是如此，在此修正之前任何已配对设备都可到达。因此网关形式改为按 namespace（命名空间）分类（`src/gateway-authority.ts`），未分类的 namespace 一律视为仅限回环，且 `verify-gateway-endpoint-authority` 会让构建失败，直到有人为新 namespace 做出明确分类。`pluginInventory` 与 `dynamicCordisRunner` 仅限回环；`commands`、`goals`、`messageFeedback` 属于配对本就涵盖的会话内容。
