...
- **Admission**: the Host fence (`src/api-request-trust.ts`) runs for every request as the DNS-rebinding/cross-site defense, then `admitApiRequest` (`src/api-auth.ts`) requires the pairing token for every non-loopback **socket peer** — presented as the `dsh_auth` cookie or `Authorization: Bearer`, compared constant-time over sha256 digests. The tokenless exemption is granted only to a genuine loopback peer (`req.socket.remoteAddress`, `isLoopbackAddress`), never on the basis of a loopback-looking `Host` — on an all-interfaces bind the header is forgeable by any client that can reach the socket, so a header-based exemption is a token bypass ([the loopback-peer fix Agent Note](../bug-fix/2026-08-16-loopback-peer-not-host-header.md)). A local process (and `adb reverse`/SSH tunnels, which connect from `127.0.0.1` on the PC) stays tokenless. Dedicated `trusted-host` RPC channels ride the same admission; `loopback`-authority channels and the privileged method set stay pinned to a loopback socket peer even for authenticated callers — pairing authenticates a device, the configuration plane additionally requires being at the machine. The node layer stamps the socket-derived loopback fact onto an internal request header so the Fetch-side pins read it unforgeably.
