...
Two boundaries stay deliberately out of scope: reachability is the webserver binding's policy (`host: 127.0.0.1 | 0.0.0.0`), and authentication is its own layer above this fence — the [pairing-token decision](../feature/2026-08-15-web-lan-pairing-token-tls.md) — because the fence is a confused-deputy defense, not an auth layer. The old guard's loopback-socket check was dropped rather than generalized: with binding expressing reachability and `trustedHosts` naming remote authorities, the socket address adds nothing a header fence does not already cover.
...
- A request from a non-loopback peer passes the fence but still needs the [pairing token](../feature/2026-08-15-web-lan-pairing-token-tls.md) to be admitted, whatever authority its `Host` claims; an unauthenticated `0.0.0.0` deployment no longer exists.
