...
两条边界刻意留在范围之外：可达性由 webserver 的绑定配置（`host: 127.0.0.1 | 0.0.0.0`）控制；认证是叠在这道栅栏之上的独立一层——[配对 token 决定](../feature/2026-08-15-web-lan-pairing-token-tls.md)——因为栅栏是混淆代理人防御，不是认证层。旧守卫的回环 socket 检查被放弃而非泛化：绑定表达可达性、`trustedHosts` 点名远程权威之后，socket 地址提供不了头部栅栏覆盖不到的任何东西。
...
- 来自非回环 peer 的请求过了栅栏之后仍需[配对 token](../feature/2026-08-15-web-lan-pairing-token-tls.md) 才被放行，无论其 `Host` 声称什么权威；无认证的 `0.0.0.0` 部署已不复存在。
