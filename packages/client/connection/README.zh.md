...
在栅栏之外，指向非回环权威的请求只有在部署配置了 `pairingToken` 且请求出示了它时才被放行（`src/api-auth.ts`）；回环调用方永远不需要 token，因为本机进程本就拥有这台服务器所在的机器。token 至少 16 个 `A-Za-z0-9_-` 字符——否则加载报错——比较在 sha256 摘要上恒定时间进行，长度与前缀匹配都不会经计时泄漏。请求以 `dsh_auth` cookie 或 `Authorization: Bearer` 请求头（非浏览器客户端路径）出示它。浏览器半侧经 URL fragment 配对：打开一次 `#auth=<token>` 链接会把 token 存入 localStorage、从地址栏剥去 fragment，并在每次启动时把它重新发布为 `SameSite=Strict` 的全源 cookie，浏览器随后会把它附加到 `/api` fetch 与 WebSocket upgrade 上——fragment 本身永远不会到达任何服务器或日志。以 `authority: 'trusted-host'` 注册的专用 RPC 通道要求同样的放行；`authority: 'loopback'` 通道与特权方法集即使调用方已认证也仍钉在回环。浏览器隐私设置拒绝存储或 cookie 访问的情况在启动时被兜住：页面以未认证状态继续，回环服务对此毫无感知。`dsh --profile web --host 0.0.0.0 --pairing-token <token>` 是组合出的 LAN 部署：Web 运行时推导 LAN 权威、经 webserver 的 TLS 材料提供 HTTPS（生成由 [dsh-web-app](../../bundle/web-app/README.md) 负责），并在每次启动打印一次配对 URL。
