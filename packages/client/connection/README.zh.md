...
来自非回环 peer 的请求只有在部署配置了 `pairingToken` 且请求以 `dsh_auth` cookie 或 `Authorization: Bearer` 请求头出示它时才被放行（`src/api-auth.ts`）；比较在 sha256 摘要上恒定时间进行。token 至少 16 个 `A-Za-z0-9_-` 字符，否则加载报错。
